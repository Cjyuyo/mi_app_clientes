...
def index():
    """
    Maneja el registro simplificado de nuevos clientes.
    """
    if request.method == 'POST':
        try:
            ...
            db = get_db()
            cursor = db.cursor()
            # Inserta solo los campos del formulario simplificado
            query = """
            INSERT INTO clientes (nombre_apellido, cedula, telefono, asesor, responsable, proceso) 
            VALUES (%(nombre_apellido)s, %(cedula)s, %(telefono)s, %(asesor)s, %(responsable)s, %(proceso)s);
            """
