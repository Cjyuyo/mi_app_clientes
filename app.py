...
def consulta():
    # --- VERSIÓN DE DIAGNÓSTICO PARA LA RUTA /consulta ---
    # Esta versión no usa la base de datos para aislar el problema.
    print("DEBUG: Ejecutando la versión de diagnóstico de /consulta")
    ...
    if termino_busqueda:
        # Devolvemos un cliente de prueba para verificar que la plantilla funciona.
        dummy_pago_conciliado = {
            'id': 101, 'recibo': 1215, 'fecha_pago': datetime.now(), 'tipo_pago': 'Cuota', 
            'monto': 100.00, 'estado_pago': 'Conciliado', 'forma_pago': 'Transferencia'
        }
        dummy_pago_pendiente = {
            'id': 102, 'recibo': 1216, 'fecha_pago': datetime.now(), 'tipo_pago': 'Inscripción', 
            'monto': 50.00, 'estado_pago': 'Pendiente', 'forma_pago': 'Efectivo'
        }
        dummy_cliente = {
            'id': 1, 'nombre_apellido': 'Cliente de Prueba (Diagnóstico)', 'cedula': termino_busqueda,
            'contrato_nro': 'C-001', 'telefono': '0414-1234567', 'fecha_ingreso': datetime.now(),
            ...
            'cuotas_pagadas_progresivas': 5, 'balance_regresivo': 50.00, 
            'pagos': [dummy_pago_conciliado, dummy_pago_pendiente]
        }
        clientes_encontrados.append(dummy_cliente)
        print(f"DEBUG: Devolviendo cliente de prueba para el término: {termino_busqueda}")
    ...
    try:
        ...
        # Si la plantilla falla, veremos este error.
        print(f"ERROR: No se pudo renderizar consulta.html: {e}")
        return f"<h1>Error al renderizar la plantilla 'consulta.html'</h1><p>El error es: {e}</p>"
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if pago_a_anular['estado_pago'] == 'Pendiente':
                ...
                query_pago_anterior = """
                    ...
                    FROM pagos WHERE cliente_id = %s AND id != %s AND estado_pago = 'Conciliado' AND tipo_pago = 'Cuota' AND fecha_pago <= %s
                ...
                cur.execute("UPDATE clientes SET cuotas_pagadas_progresivas = %s, cuotas_pagadas_regresivas = %s, balance_regresivo = %s WHERE id = %s",
